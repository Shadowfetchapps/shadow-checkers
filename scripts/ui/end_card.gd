...
func _init() -> void:
	...
	row2.add_child(UIKit.button("Save PDN", func(): export_requested.emit(), "GhostButton", "download"))
...
func present(info: Dictionary, is_ai: bool) -> void:
	...
	if is_ai:
		...
		if winner < 0:
			...
			headline = "%s wins" % MaterialLibrary.side_label(winner)
			icon = "trophy"
	...
	_stats.text = "%d turn%s  ·  %s" % [n, "" if n == 1 else "s", str(info.get("reason", ""))]
	_swap.visible = is_ai
	open()
