...
## Shadowfetch club-room UI theme: green-black lacquer panels, brass accents.
## Same structure as Shadow Chess 3D so both games feel like one family.
## One Theme instance is built lazily and
## shared by every screen; type variations cover buttons, labels, and panels.

const GOLD := Color(0.86, 0.70, 0.40)
const GOLD_BRIGHT := Color(0.96, 0.86, 0.58)
const GOLD_DIM := Color(0.56, 0.46, 0.24)
const CREAM := Color(0.95, 0.93, 0.86)
const MUTED := Color(0.64, 0.67, 0.61)
const FAINT := Color(0.43, 0.47, 0.43)
const INK := Color(0.035, 0.052, 0.046, 0.94)
const INK_SOLID := Color(0.028, 0.04, 0.036)
const CARD := Color(0.058, 0.08, 0.07, 0.96)
const EMERALD := Color(0.42, 0.86, 0.64)
...
static func _build() -> Theme:
	...
	t.set_stylebox("panel", "CardActive", box(Color(0.09, 0.125, 0.10, 0.97), Color(GOLD.r, GOLD.g, GOLD.b, 0.75), 12, 1, Vector4(14, 12, 14, 12)))
	t.set_type_variation("Modal", "PanelContainer")
	t.set_stylebox("panel", "Modal", _shadowed(box(Color(0.042, 0.06, 0.053, 0.985), Color(GOLD.r, GOLD.g, GOLD.b, 0.40), 18, 1, Vector4(28, 24, 28, 24)), 40, 0.6))
	t.set_type_variation("Pill", "PanelContainer")
	t.set_stylebox("panel", "Pill", _shadowed(box(Color(0.04, 0.058, 0.05, 0.92), Color(GOLD.r, GOLD.g, GOLD.b, 0.30), 999, 1, Vector4(18, 7, 18, 7)), 14, 0.35))
	t.set_type_variation("Toast", "PanelContainer")
	t.set_stylebox("panel", "Toast", _shadowed(box(Color(0.07, 0.10, 0.085, 0.97), Color(GOLD.r, GOLD.g, GOLD.b, 0.55), 12, 1, Vector4(16, 10, 16, 10)), 20, 0.5))
	t.set_type_variation("Inset", "PanelContainer")
	t.set_stylebox("panel", "Inset", box(Color(0.02, 0.03, 0.026, 0.75), Color(1, 1, 1, 0.04), 10, 1, Vector4(10, 8, 10, 8)))
	t.set_type_variation("Clear", "PanelContainer")
	t.set_stylebox("panel", "Clear", StyleBoxEmpty.new())

	# --- Buttons ------------------------------------------------------------
	_button_style(t, "Button",
		Color(0.075, 0.10, 0.088, 0.96), Color(GOLD.r, GOLD.g, GOLD.b, 0.32),
		Color(0.11, 0.15, 0.12, 0.98), Color(GOLD.r, GOLD.g, GOLD.b, 0.85),
		Color(0.16, 0.21, 0.15, 1.0), CREAM, GOLD_BRIGHT, 8, Vector4(16, 9, 16, 9))
	...
	_button_style(t, "MainMenuButton",
		...
		Color(0.86, 0.70, 0.40, 0.10), Color(GOLD.r, GOLD.g, GOLD.b, 0.0),
		Color(0.86, 0.70, 0.40, 0.18), CREAM, GOLD_BRIGHT, 10, Vector4(18, 11, 18, 11))
	t.set_font("font", "MainMenuButton", font("display_medium"))
	t.set_font_size("font_size", "MainMenuButton", 20)
	var menu_hover := box(Color(0.86, 0.70, 0.40, 0.10), Color(0, 0, 0, 0), 10, 0, Vector4(18, 11, 18, 11))
	...
	_button_style(t, "IconButton",
		...
		Color(0.86, 0.70, 0.40, 0.14), Color(GOLD.r, GOLD.g, GOLD.b, 0.6),
		Color(0.86, 0.70, 0.40, 0.24), CREAM, GOLD_BRIGHT, 9, Vector4(9, 9, 9, 9))

	t.set_type_variation("ChipButton", "Button")
	_button_style(t, "ChipButton",
		Color(1, 1, 1, 0.035), Color(1, 1, 1, 0.08),
		Color(0.86, 0.70, 0.40, 0.10), Color(GOLD.r, GOLD.g, GOLD.b, 0.5),
		Color(0.86, 0.70, 0.40, 0.22), MUTED, CREAM, 9, Vector4(14, 9, 14, 9))
	var chip_on := box(Color(0.86, 0.70, 0.40, 0.20), GOLD, 9, 1, Vector4(14, 9, 14, 9))
	...
	_button_style(t, "MoveButton",
		...
		Color(0.86, 0.70, 0.40, 0.22), CREAM, GOLD_BRIGHT, 6, Vector4(8, 3, 8, 3))
	t.set_font("font", "MoveButton", font("tabular"))
	t.set_font_size("font_size", "MoveButton", 14)
	var mv_on := box(Color(0.86, 0.70, 0.40, 0.26), Color(GOLD.r, GOLD.g, GOLD.b, 0.7), 6, 1, Vector4(8, 3, 8, 3))
	...
	var opt_n := box(Color(0.07, 0.095, 0.083, 0.96), Color(1, 1, 1, 0.08), 8, 1, Vector4(12, 8, 30, 8))
	var opt_h := box(Color(0.10, 0.14, 0.11, 0.98), Color(GOLD.r, GOLD.g, GOLD.b, 0.6), 8, 1, Vector4(12, 8, 30, 8))
	...
	var popup := _shadowed(box(Color(0.05, 0.07, 0.06, 0.99), Color(GOLD.r, GOLD.g, GOLD.b, 0.35), 10, 1, Vector4(6, 6, 6, 6)), 20, 0.5)
	t.set_stylebox("panel", "PopupMenu", popup)
	t.set_stylebox("hover", "PopupMenu", box(Color(0.86, 0.70, 0.40, 0.18), Color(0, 0, 0, 0), 6, 0, Vector4(8, 4, 8, 4)))
	...
	t.set_stylebox("panel", "TooltipPanel", box(Color(0.07, 0.10, 0.0855, 0.98), Color(GOLD.r, GOLD.g, GOLD.b, 0.45), 8, 1, Vector4(10, 6, 10, 6)))
	...
	var tab_sel := box(Color(0.86, 0.70, 0.40, 0.16), Color(0, 0, 0, 0), 8, 0, Vector4(16, 8, 16, 8))
...
static func _field(focus: bool) -> StyleBoxFlat:
	return box(Color(0.025, 0.035, 0.03, 0.95), GOLD if focus else Color(1, 1, 1, 0.08), 8, 1, Vector4(10, 8, 10, 8))
