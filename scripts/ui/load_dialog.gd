...
## Saved games with open/delete, plus PDN import from a file.
...
func _init() -> void:
	...
	add_footer_button(UIKit.button("Import PDN file…", _import_pgn, "", "upload"))
...
func _populate() -> void:
	...
	if saves.is_empty():
		...
		empty.add_child(l)
		var l2 := UIKit.label("Use Save game from the in-game menu, or import a PDN file.", "Caption")
...
func _import_pgn() -> void:
	if _file_dialog == null:
		...
		_file_dialog.filters = PackedStringArray(["*.pdn ; PDN draughts game", "*.txt ; Text"])
		_file_dialog.title = "Import PDN"
		_file_dialog.file_selected.connect(func(path: String):
			...
			f.close()
			var probe := CheckersPdn.import_game(text)
			if not bool(probe.get("ok", false)):
				set_subtitle("Could not read that PDN: %s" % str(probe.get("error", "")))
				return
