...
func set_eval(white_cp: int, mate_white: int) -> void:
	...
	if _mate:
		...
		_text = "W%d" % absi(mate_white)
	elif absi(white_cp) >= 9000:
		_goal = 1.0 if white_cp > 0 else 0.0
		_text = "1-0" if white_cp > 0 else "0-1"
	else:
		_goal = 1.0 / (1.0 + exp(-float(white_cp) / 180.0))
		var pawns := absf(white_cp / 100.0)
		_text = "%s%.1f" % ["+" if white_cp > 0 else ("−" if white_cp < 0 else ""), pawns]
	tooltip_text = "Evaluation %s men (%s)" % [_text, "%s better" % MaterialLibrary.side_label(CheckersTypes.WHITE) if _goal > 0.5 else ("%s better" % MaterialLibrary.side_label(CheckersTypes.BLACK) if _goal < 0.5 else "equal")]
