...
var side: int = CheckersTypes.WHITE
...
func setup(p_side: int, display_name: String, subtitle: String, is_ai: bool) -> void:
	...
	_sub.text = subtitle
	var disc := Color(0.93, 0.89, 0.80) if side == CheckersTypes.WHITE else Color(0.08, 0.07, 0.07)
	...
	if is_ai:
		_avatar_icon.texture = IconLibrary.get_icon("moon", 20, ThemeFactory.GOLD if side == CheckersTypes.BLACK else Color(0.25, 0.2, 0.12))
		...
		_avatar_label.add_theme_color_override("font_color", Color(0.12, 0.1, 0.07) if side == CheckersTypes.WHITE else ThemeFactory.CREAM)
...
## `types` are the piece types this side has captured; shown as disc icons
## with counts, plus the live piece balance.
func set_captured(types: Array, diff: int) -> void:
	for c in _captured.get_children():
		c.queue_free()
	var opp := CheckersTypes.opp(side)
	var men := 0
	var kings := 0
	for t in types:
		if int(t) == CheckersTypes.KING:
			kings += 1
		else:
			men += 1
	for pair in [[CheckersTypes.MAN, men], [CheckersTypes.KING, kings]]:
		if int(pair[1]) == 0:
			continue
		var tex := PieceIcons.get_icon(int(pair[0]), opp)
		if tex:
		...
		var l := UIKit.label("×%d" % int(pair[1]), "Caption")
