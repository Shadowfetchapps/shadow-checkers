...
func _build_setup() -> PanelContainer:
	...
	p.custom_minimum_size = Vector2(420, 380)
	p.offset_left = -210
	p.offset_right = 210
	p.offset_top = -210
	p.offset_bottom = 210
	...
	_color.add_item("Play Black (moves first)", 0)
	_color.add_item("Play White", 1)
...
func _start_ai() -> void:
	GameSession.configure_ai(false)
	get_tree().change_scene_to_file("res://scenes/main/game.tscn")
...
func _start_setup() -> void:
	...
	if _mode.selected == 1:
		GameSession.configure_ai(_color.selected == 1)
	else:
