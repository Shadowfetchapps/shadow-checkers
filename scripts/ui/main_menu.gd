...
## Title screen: the living club-room backdrop with the menu column on the left.
...
func _build() -> void:
	...
	_column.add_child(UIKit.label("SHADOWFETCH  ·  CLUB ROOM", "Kicker"))
	var title := UIKit.label("Shadow Checkers", "HeaderXL")
	...
	_column.add_child(UIKit.label("Three rule sets. Green baize. One sharp opponent.", "Muted"))
	...
	if SaveManager.has_autosave():
		...
		desc = "%s · %s" % [CheckersRules.variant_name(str(data.get("variant", "english"))).replace(" Draughts", ""), desc]
		var moves: Array = data.get("moves_uci", data.get("legacy_hops", []))
		b.tooltip_text = "%s · turn %d" % [desc, moves.size()]
		_column.add_child(b)
		var sub := UIKit.label("     %s  ·  turn %d" % [desc, moves.size()], "Faint")
	...
	_column.add_child(UIKit.label("Version %s  ·  Linux  ·  English, Russian & Brazilian rules" % ver, "Faint"))

	var credit := UIKit.label("On the board: Shadow playing itself, live", "Faint")
