...
## Rating against Shadow, record per rules variant and level, lifetime counters.


func _init() -> void:
	super("Statistics", 680, true, 470)
	...
	for v in CheckersRules.variants():
		...
		body.add_child(UIKit.label(str(v.get("name")).to_upper(), "Kicker"))
		...
		for l in CheckersAI.levels():
			var id := str(l.get("id"))
			var rec: Dictionary = ProfileStore.levels.get("%s:%s" % [vid, id], {})
			var w := int(rec.get("w", 0))
			var d := int(rec.get("d", 0))
			var lo := int(rec.get("l", 0))
			var n := w + d + lo
			grid.add_child(UIKit.label(str(l.get("name"))))
		...
		body.add_child(grid)
	body.add_child(UIKit.separator())
	body.add_child(UIKit.label("Turns played %d   ·   Hints used %d   ·   Best win streak %d   ·   Local games %d" % [ProfileStore.moves_played, ProfileStore.hints_used, ProfileStore.best_streak, ProfileStore.local_games], "Caption", true))
