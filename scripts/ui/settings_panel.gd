...
func _game_page(page: VBoxContainer) -> void:
	...
	for v in CheckersRules.variants():
		vids.append(str(v.get("id")))
		vnames.append(str(v.get("name")))
	page.add_child(UIKit.row("Default rules", UIKit.option(vnames, vids, SettingsStore.variant, func(v): _apply_setting("variant", v))))
	...
	_section(page, "Play")
	page.add_child(UIKit.toggle("Finish forced jump sequences automatically", SettingsStore.auto_complete, func(v): _apply_setting("auto_complete", v)))
...
func _board_page(page: VBoxContainer) -> void:
	...
	page.add_child(UIKit.toggle("Ring the pieces that can move", SettingsStore.show_movable, func(v): _apply_setting("show_movable", v)))
	page.add_child(UIKit.toggle("Show square numbers (1–32)", SettingsStore.show_square_numbers, func(v): _apply_setting("show_square_numbers", v)))
