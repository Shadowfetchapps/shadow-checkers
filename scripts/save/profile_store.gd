...
## Player profile: results against each Shadow level (per rules variant), an
## Elo-style rating against Shadow's nominal strengths, and lifetime counters.
...
const LEVEL_ELO := {
	"beginner": 700, "casual": 1000, "club": 1350,
	"advanced": 1650, "expert": 1900, "master": 2150,
}
...
func record_ai_result(level: String, score: float) -> float:
	...
	if score >= 1.0:
		...
		rec["d"] = int(rec["d"]) + 1
	levels[level] = rec
	# Keys may be "variant:level"; the nominal strength depends on the level.
	var opp := float(LEVEL_ELO.get(level.get_slice(":", level.get_slice_count(":") - 1), 1350))
