class_name GameClockProp
extends Node3D

## Decorative tournament clock on the table (same prop family as Shadow Chess). The controller feeds it live
...
func set_times(white_s: float, black_s: float, running: int, enabled: bool) -> void:
	...
	_btn_left.position.y = 0.325 if running == CheckersTypes.WHITE else 0.35
	_btn_right.position.y = 0.325 if running == CheckersTypes.BLACK else 0.35
	_lamp_left.material_override = _lamp_on if running == CheckersTypes.WHITE and enabled else _lamp_off
	_lamp_right.material_override = _lamp_on if running == CheckersTypes.BLACK and enabled else _lamp_off
